...
2. Some panel functions like Add Panel and Remove Panel REALLY SHOULD be deleted for stability - procedures are below.<br>
3. Nothing is dangerous when everything is considered in proper context.<br>
4. When a user toggles hidden files in Nemo, Nemo simply closes and restarts. Not a perfect solution but should keep kids from accidentally deleting hidden files.<br>
5. You can set a limit to the number of workspaces a user may create.<br>
6. At anytime you can stop Nemo Hidden closing/restarting & Workspace Max Number Limiting by typing into a terminal:# killall dbus-monitor.
<br>Logout and then Login and they will be running again. 
<br>
<br>
At time of writing I am using Debian 12.6, this is the minimum so DO an apt update && upgrade to make sure you are current.<br>
...
3. Trigger actions: Includes Enable Default Applets And Set Cinnamenu As The Default Main Menu <br>
4. Filename ~/.config/firstlogincomplete_DONOTDelete will now get created, so going forward trigger functions are not applied as file now exists.<br>
